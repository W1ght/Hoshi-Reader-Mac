...
- Google token：`Features/Sync/TokenStorage.swift`，存入 Application Support 下的 `google_drive_tokens.json`；token 与同文件内本地密钥配合做低安全加密存储，以避免 Keychain 授权弹窗。不得重新引入 Keychain token 存储，除非同时设计提示、迁移和用户重连策略。
