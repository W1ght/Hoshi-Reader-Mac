...
d_m3Op  (Loop)
{
    ...
    do
    {
        // Niratan runs untrusted source modules. Check the runtime's atomic
        // watchdog once per loop back-edge so a tight WASM loop cannot pin the
        // app without ever reaching Wasm3's normal call-boundary yield.
        m3ret_t possible_trap = m3_Yield ();
        if (M3_UNLIKELY(possible_trap)) return possible_trap;
#if d_m3EnableStrace >= 3
        d_m3TracePrint("iter {");
