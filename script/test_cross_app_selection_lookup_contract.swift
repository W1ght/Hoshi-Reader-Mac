...
private enum CrossAppSelectionLookupContractTests {
    static func main() throws {
        ...
        expect(reader.contains("screenBounds = selectedTextScreenBounds(focusedElement: focusedElement)"), "reader must capture selected-text bounds before deciding whether AX text requires fallback")
        expect(reader.contains("copyFallback.readSelectedText(screenBounds: screenBounds)"), "copy fallback snapshots should keep AX selected-text bounds so panels anchor to the highlighted word")
