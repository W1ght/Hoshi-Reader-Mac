...
- Windowed playback uses mpv's display dimensions to keep the dedicated player window matched to the video's effective aspect ratio. When the study sidebar is visible, the window and sidebar width participate in that same ratio so the video surface stays filled instead of showing ambient or black bars. Full screen releases the windowed aspect constraint and keeps normal pure-black letterboxing; the existing mpv video-only screenshot path remains isolated from window chrome and subtitles.
...
- Windowed playback should avoid letterbox space by locking the dedicated window to the current video's effective aspect ratio. If the study sidebar is visible, resize the sidebar or window together so the video area stays aspect-correct. Full screen keeps normal pure-black letterboxing and does not use ambient blur.
