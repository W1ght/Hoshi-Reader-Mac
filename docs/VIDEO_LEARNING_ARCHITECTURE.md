...
- Existing AnkiConnect mining with video file, timestamp, cue, adjacent-cue handlebars, mpvacious-style current-frame screenshot and subtitle-time-range audio capture. The shared image compression setting selects JPEG for the current-frame path or an animated AVIF covering the subtitle range; the animated path has mpv decode, scale and emit YUV420 frames directly before the bundled SVT-AV1 encoder and AVIF muxer, without an intermediate PNG sequence. Local/generated media prefers direct writes into Anki's `collection.media` directory with `[sound:filename]` and `<img src="filename">` field markup; remote word audio remains on the existing AnkiConnect/base64 attachment path. Mining History is independent of this result path: it saves the current visible subtitle first and later returns the user to the shared lookup/Popup mining flow.
