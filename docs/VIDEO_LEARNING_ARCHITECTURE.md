...
The Video render bridge follows the IINA-style libmpv embedding path directly: an `NSView` host owns a `CAOpenGLLayer`, mpv render updates enter a dedicated layer GL queue, `MPV_RENDER_PARAM_ADVANCED_CONTROL` drives `mpv_render_context_update`, frame draws render to the layer FBO with depth metadata, and missed layer draws are consumed with `MPV_RENDER_PARAM_SKIP_RENDERING`. Do not reintroduce the old `NSOpenGLView.drawRect` path, main-thread `setNeedsDisplay` render callbacks, or software/legacy pixel-format fallbacks.
...
- External subtitle timing follows the same delay as mpv; selecting an external subtitle disables the embedded subtitle track to prevent duplicate rendering. When a local media file has an mpv-style same-name `.srt`/`.vtt`/`.ass`/`.ssa` sidecar, Hoshi also parses that sidecar into its own transcript/lookup overlay so seeking to an arbitrary time can immediately show nearby subtitle rows instead of waiting for mpv's current-line snapshots.
...
- Embedded and mpv-loaded external text subtitle tracks are mapped from mpv's `sub` track metadata and fully extracted with the bundled FFmpeg libraries when selected. Embedded ASS/SSA extraction retains CodecPrivate headers and raw packet event metadata so it can rebuild the same document used by external ASS/SSA. Ordinary text and bottom-aligned ASS/SSA primary dialogue render through the interactive Niratan overlay; authored positioning, lyric/OP/ED events, animation, karaoke, drawings and duplicate effects render through a hidden internal effects-only libass track. The public track list keeps the original logical track selected and never exposes that implementation track. Image-based subtitles expose an unsupported text-list state rather than mixing stale transcript rows from another track.
- Subtitle appearance controls set Niratan-owned text subtitle font, size, text color and lookup-highlight background color. Defaults follow asbplayer text subtitle semantics: empty font family means system default, subtitle size starts at 36 px, text is white and lookup highlighting uses the macOS selection color. Subtitle vertical position follows IINA/mpv's relative-position model through one unlabeled top-to-bottom slider backed by a normalized `0...1` value. The overlay measures the complete subtitle stack and places its top at `max(viewportHeight - subtitleHeight, 0) * position`, so both endpoints keep the stack visible when it fits and do not depend on playback-control clearance. The value persists under `videoSubtitleVerticalPositionFraction`; the former `videoSubtitleVerticalPosition` point value is only read for deterministic migration and is not deleted. Ordinary ASS/SSA primary dialogue uses this same visible TextKit layout, so glyph drawing, native drag selection, click and Shift-hover lookup, highlight geometry and popup anchoring move together with the slider. Author-positioned effects stay fixed in libass and intentionally do not inherit the slider. The playback bridge publishes libmpv's actual `osd-dimensions` surface size and top/bottom/left/right video margins; the overlay scales that geometry into its SwiftUI surface and frames itself inside the resulting viewport. Before valid OSD geometry arrives, the same effective media/override/rotation ratio used by window sizing provides a temporary aspect-fit fallback; there is no full-screen-specific compensation branch. Subtitle mask controls can blur or make Niratan-owned text subtitles transparent until pointer hover. The controls live in both Video Settings and the inspector's Subtitles tab, update the AppKit subtitle view live, and do not create a separate rectangular blur overlay.
- Native macOS media presentation requires macOS 26+ and uses floating Liquid Glass control surfaces with restrained custom chrome intended to remain visually compatible with macOS 27.
...
- Video UI follows a Hoshi learning-player direction rather than a general-purpose player skin: Floating is a compact IINA-like two-row Liquid Glass surface that remains draggable within the video surface, while Compact Bottom always derives its position from the current video-surface bottom and never consumes a Floating drag offset. Both are revealed by pointer movement over the video and hidden automatically after a short idle delay, pointer exit, or app deactivation. Play/pause and speed use plain foreground controls with lightweight pressed feedback instead of nested persistent backgrounds.
- Video uses one AppKit-owned, non-restoring player window. Selecting Video in the main sidebar first presents the shared media picker; a successful request creates or focuses that window, and later requests replace its media after saving state. The window keeps standard macOS titlebar and traffic-light behavior, declares `fullScreenPrimary`, and owns the complete mpv lifecycle. Closing it saves state, shuts down mpv and does not continue playback in the background.
- Speed, timing, media tracks, external subtitles, episode selection and playback options live in a right-side IINA-inspired Liquid Glass inspector with `Episodes`, `Video`, `Audio` and `Subtitles` sections. The bottom speed button opens its compact adjustment panel through player-surface-owned presentation state; the existing canvas dismissal layer sits below the controls so panel interaction remains active and a click elsewhere on the video closes it without an AppKit-wide event monitor. The inspector's subtitle action opens the fixed study sidebar directly on Transcript; chapter navigation lives only in that study sidebar. Mining History and Open Video are first-class actions in the widened bottom playback bar rather than top-left overlays, leaving the native traffic lights unobstructed. The inspector overlays the trailing edge of the video surface and may cover the picture like IINA; it must not take side-by-side layout space or resize the media canvas. Inspector tabs and multi-choice controls should reuse the same glass segmented/pill language as native Settings controls instead of plain bordered controls. IINA is only a UX reference; Hoshi does not copy IINA source, assets, or app architecture.
- Player shutdown and security-scoped URL cleanup occur when the dedicated Video window closes. Reopening creates a new player session and restores per-file state; multiple simultaneous Video windows are intentionally unsupported. A window-scoped chrome controller targets that exact window, tracks AppKit full-screen will/did enter/exit notifications, blocks chrome/aspect/frame mutation while AppKit is transitioning, and keeps the standard traffic lights available for system window management. A main-window open request remains pending until `MpvRenderView` confirms that libmpv attached to its OpenGL surface, then loads and consumes the request exactly once; inspector, picker and drop replacements continue through the same already-ready player path.
- SRT/VTT parsing plus ASS/SSA event/style parsing, primary-dialogue classification, effects-only reconstruction, overlapping cue lookup and the Niratan-owned interactive subtitle overlay.
...
- Existing AnkiConnect mining with video file, timestamp, cue, adjacent-cue handlebars, mpvacious-style current-frame screenshot and subtitle-time-range audio capture. The shared image compression setting selects JPEG for the current-frame path or an animated AVIF covering the subtitle range; the animated path streams scaled raw 10-bit YUV420 frames straight out of mpv (rawvideo, not the 8-bit-only yuv4mpegpipe text layer) into the bundled SVT-AV1 encoder and AVIF muxer, so HEVC/x265 10-bit sources keep their bit depth. Local/generated media prefers direct writes into Anki's `collection.media` directory with `[sound:filename]` and `<img src="filename">` field markup; remote word audio remains on the existing AnkiConnect/base64 attachment path. Mining History is independent of this result path: it saves the current visible subtitle first and later returns the user to the shared lookup/Popup mining flow.
- Video card setup uses the normal Anki field mapping UI and the same `Japanese` Profile as EPUB. The one default mapping uses `{book-cover}` and `{sasayaki-audio}`; in a Video mining context those handles resolve to the current frame and the delayed, padded subtitle audio range, while EPUB keeps its book-cover and Sasayaki semantics. Legacy `{video-screenshot}` and `{video-audio-clip}` mappings remain supported. Add to Anki checks configuration and duplicates before preparing media. Video media is prepared only when either the shared or legacy handle is mapped. If Anki's media directory is available, deterministic source-path-SHA filenames are exposed only after the required screenshot and audio files have been written successfully; repeated requests for the same media wait for the shared in-flight generation. If the media directory is unavailable, Niratan falls back to the previous AnkiConnect attachment path. In either mode, a failed required mapped screenshot or audio clip stops card creation before `addNote`.
- A fixed right-side study sidebar switches between Mining History, Transcript and Chapters with a native glass segmented control and pushes the video surface instead of covering the picture. The sidebar itself may use one material/glass surface, but its long scrolling rows must use lightweight tint/stroke cards rather than per-row `glassEffect`, because playback-time updates, transcript following and mouse scrolling otherwise force expensive glass recomposition. Mining History lists chronological subtitle captures in contiguous source sections with card jump plus direct copy/delete and confirmed clear actions; Transcript seeks by subtitle time; Chapters highlights the current marker and seeks through the existing playback engine. The configurable history limit defaults to 25, and zero disables and clears history.
...
- On-demand libmpv frame capture and bundled-libmpv AAC/M4A audio-clip export for mapped Video media fields. Video subtitle mining checks duplicate/configuration state first, then captures only the media requested by field mappings. Both direct-media and fallback attachment modes treat missing required mapped media as a pre-submit failure.
- Full-build package and release contracts.

## Deferred Scope

Secondary-track subtitle import, embedded secondary-subtitle extraction, character hit geometry for explicitly positioned ASS effects, viewing statistics and sync remain future work.
...
| P0 | External SRT/VTT plus external/embedded ASS/SSA primary-dialogue lookup overlay | Implemented; explicitly positioned effect glyph lookup remains deferred |
...
### Video Full-Screen Regression Notes

The 2026-06 full-screen crash/freeze presented when exiting native macOS full screen with `Esc`, `f`, the bottom full-screen button, or the system green traffic light. Crash reports pointed at AppKit's `_NSExitFullScreenTransitionController` while it was preparing the post-full-screen resize snapshot. The regression was narrowed by comparing the beta-era git changes and by checking IINA's behavior: IINA uses AppKit `window.toggleFullScreen`, explicit will/did full-screen state, a stable render view, and avoids frame/aspect mutations during the transition.
...
Computer Use verification needs a precise trigger loop because both the bottom OSC and system traffic lights can disappear before the model's next action. Reveal the needed chrome with pointer movement, immediately re-query UI state, click the returned control from that same snapshot, then wait for the Space transition and re-query before the next action. If the control vanishes, reveal and query again; old element ids or coordinates are not reliable evidence for this bug.

## Visual Contract

- Treat the video page as a Hoshi learning surface, not a general-purpose player skin.
- Keep standard macOS window management discoverable on the Video page. The bottom OSC can auto-hide, but the native titlebar/traffic-light path must remain compatible with system full screen.
...
- The inspector and study sidebar should read as Liquid Glass without stacking expensive glass samplers over the live video frame: keep one glass/material outer surface per panel, then use lightweight tint/stroke section cards, list rows, segmented choices and action buttons inside it. Avoid ordinary bordered buttons or system segmented controls that visually flatten the panel, but do not restore per-section/per-row `glassEffect` layers.
...
- Subtitle text remains on a transparent overlay without a glass, material, black, or opaque background frame; its wrapping width, horizontal center and vertical-position origin follow the aspect-fit visible video viewport rather than the outer window. Lookup rectangles remain in the shared `video-player` coordinate space so popup presentation is unchanged. Subtitle appearance is limited to text font/size, and subtitle masking is limited to blur/opacity effects on the text row that must reveal on pointer hover.
...
- New custom glass elements can use macOS 26+ Liquid Glass APIs directly. Do not add older macOS material fallbacks unless the deployment target is explicitly lowered again.
