...
- Windowed playback uses mpv's display dimensions for one-shot fitting of the dedicated player window to the video's effective aspect ratio. Hoshi does not leave a persistent `NSWindow.contentAspectRatio` constraint installed, because AppKit full-screen exit snapshots can crash or freeze when the window is constrained and being resized during the system transition. Full screen keeps normal pure-black letterboxing; the existing mpv video-only screenshot path remains isolated from window chrome and subtitles.
...
The stable Hoshi chain is now: `VideoWindowPresenter` owns a normal AppKit `NSWindow` with `fullScreenPrimary`; `VideoWindowChromeController` is the single full-screen state source; the controller clears any old aspect constraint, ignores repeated toggles while entering/exiting, and only applies one-shot windowed aspect fitting outside full screen; `MpvRenderView` remains attached instead of being removed or blacked out for transition workarounds; custom fullscreen controls, double-click, `f`, and `Esc` all route through the same window toggle/exit path. Future changes must not reintroduce a persistent `contentAspectRatio`, `setFrame`, SwiftUI window identity churn, or mpv render detach inside AppKit full-screen will/exit notifications.
...
- Windowed playback should avoid letterbox space by fitting the dedicated window to the current video's effective aspect ratio when media/sidebar state changes, without leaving a persistent AppKit aspect-ratio lock installed. Full screen keeps normal pure-black letterboxing and does not use ambient blur.
