...
- Video variants: the native target has Light (`Debug`/`Release`) and Video (`Debug-Video`/`Release-Video`) configurations. Light excludes Video/libmpv; Video provides local playback, SRT/VTT/ASS/SSA text overlay, shared popup/nested lookup/local word audio, and video-specific Anki mining fields.
...
- Video subtitle appearance controls set Niratan-owned text subtitle font, size, edge style and edge strength, defaulting to the system font, 36 px size and a strong high-contrast glyph outline for new users. Soft Shadow uses a darker zero-offset glyph shadow, while Clear Outline and High Contrast provide two stable outline weights. Subtitle mask controls can blur or hide text subtitles until pointer hover. These settings are shared between Video Settings and the playback inspector and remain text-only overlay effects without adding a subtitle background frame.
