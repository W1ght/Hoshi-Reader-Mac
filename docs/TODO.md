...
- App bundle identity is now `moe.shishamo.hoshi` for both Light and Video variants. Google Drive keychain token reads and writes are aligned with upstream account-only storage for `accessToken`, `refreshToken`, and `clientId`; do not reintroduce a Mac-only Google Drive service namespace without a prompt/continuity migration plan.
