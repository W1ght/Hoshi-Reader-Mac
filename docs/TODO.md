...
- Manually verify sidebar file selection/cancel, dedicated Video window creation/focus/replacement/close/reopen, key-window Profile and shortcut isolation, video playback/seek, draggable chrome auto-hide, pointer playback/fullscreen gestures, video/subtitle drag-and-drop, subtitle lookup/appearance/mask, nested popup, local audio, screenshot/audio mining, Mining History and disposable-deck Anki outcomes before shipping. Primary SRT import, transcript performance and bundled-libmpv MKV audio export have focused checks, but the independent-window lifecycle and media/Anki UI paths still need a disposable-data manual pass.
