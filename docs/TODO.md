...
- Mac native migration: the screen-rewrite attempt was discarded; this is a Mac-only product. Anki follows the Mac AnkiConnect path, settings/bookshelf shed desktop-layout gates, LocalFileServer no longer uses iOS background tasks, update UI uses the Mac path directly, sync token fallback storage is Mac-only, `AppPlatform` now reflects Mac-only constants, cover images no longer depend on `UIImage`, dictionary page layout uses explicit Mac insets, app startup no longer installs iOS quick-action scene delegates, Reader resign-active autosync no longer uses an iOS background task wrapper, Sasayaki auto-scroll playback uses a Mac activity token, and the remaining UIKit/Catalyst inventory is tracked in `docs/MAC_NATIVE_MIGRATION_INVENTORY.md`.
...
- Next low-risk candidates: CSS editor selection bridge, keyboard shortcut capture bridge, then dictionary search field focus/input mode.
