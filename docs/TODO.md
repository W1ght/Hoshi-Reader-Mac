...
- Video subtitle mask controls can blur or hide Hoshi-owned text subtitles until pointer hover. The setting is shared between Video Settings and the playback inspector and remains a text-only overlay effect without adding a subtitle background frame.
