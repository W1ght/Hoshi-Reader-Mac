...
- Lookup interaction: Reader and Popup restore the v0.5 Shift-hover scan path, while Video subtitles use the same point-to-character path for click and Shift-hover. Holding Shift while moving performs continuous debounced lookup using the shared `Mac Hover Delay`; actual EPUB/video pointer behavior still needs an in-App manual pass.
