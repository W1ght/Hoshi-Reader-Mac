...
- Keep all local and remote Manga entry points behind `MangaReadingSession` and `MangaPageContentProvider`. A remote card or chapter click first hands a lightweight `MangaRemoteReadingRequest` to the AppKit-owned reader window; that window owns the cancellable async bootstrap and shows its loading/error/retry state while Suwayomi prepares details, chapters and initial pages, then replaces only its content with the shared `MangaReaderView`. Do not make the source sheet wait for page preparation or create a second reader. The native reader, OCR, Popup, nested lookup, and Anki mining must not depend on a local library item or on Suwayomi transport details.
- Keep external extension execution outside Niratan. `SuwayomiClient` is a narrow connector to an existing Server's REST/GraphQL authentication surface; Niratan must not bundle Java, execute Mihon APKs, restore the removed Shinsou JavaScript/XPC runtime, add AidokuRunner/`.aix`, or introduce a second reader.
- Let Suwayomi own extension installation/configuration, source HTTP behavior, cookies, anti-bot handling, library membership, and chapter progress. Niratan stores only Profile-scoped connection metadata, Keychain credentials, and a bounded Profile/server-isolated page cache, and reaches source images only through Suwayomi's authenticated thumbnail/page proxy.
- Keep `manga_library.json` local-only. The online library remains server-authoritative and must never be merged into, overwrite, or delete local Manga catalog records.
