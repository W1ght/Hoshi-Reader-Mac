...
- Keep subtitle appearance and masking as Niratan-owned text overlay effects: font/size controls, glyph-level shadow/outline, blur and opacity can change text rendering, but they must not add background frames, depend on mpv-rendered subtitles, or become a draggable rectangular blur overlay.
