...
- chapter-list, highlight, character-count, and internal-link jumps: restore the prior position with the backward progress control, return with the forward control, confirm manual page turns or continuous scrolling invalidate the stale forward destination, and cover both same-chapter and cross-chapter targets in paginated and continuous modes;
